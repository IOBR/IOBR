...
#'
#' @examples
find_outlier_samples<- function(eset, yinter = -3, project = "find_outlier_eset", plot_hculst = FALSE, show_plot = TRUE){
  ...
  p<- p + design_mytheme(axis_angle = 0, axis_text_size = 6)
  if(show_plot) print(p)

  ggsave(p, filename = paste0("2-connectivityplot.pdf"), width = 8, height = 8, path = path$folder_name)
