...
IPS_calculation <- function(project = NULL, eset, plot = FALSE) {
  ...
  for (i in 1:ncol(eset)) {
    ...
    if (plot) {
      ...
      plot_a5 <- plot_a4 + ggplot2::geom_text(ggplot2::aes(x = 0, y = 6, label = paste("Immunophenoscore: ", IPS[i], sep = "")), angle = 0, size = 6, vjust = -0.5) + ggplot2::theme(axis.title = ggplot2::element_blank())
      plot_a <- plot_a5 + ggplot2::theme(plot.margin = grid::unit(c(0, 0, 0, 0), "mm")) + ggplot2::geom_text(vjust = 1.15, hjust = 0, ggplot2::aes(x = 25.5, y = 6, label = "\n\n\n\n   MHC: Antigen Processing                                 EC: Effector Cells\n   CP: Checkpoints | Immunomodulators              SC: Suppressor Cells\n\n", hjust = 0), size = 4)
      ...
      legendtheme <- ggplot2::theme(plot.margin = grid::unit(c(2, 0, 2, 0), "inch"), panel.margin = grid::unit(0, "null"), panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), panel.border = ggplot2::element_blank(), panel.background = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "white"), axis.text = ggplot2::element_blank(), axis.ticks = ggplot2::element_blank(), axis.title.x = ggplot2::element_blank())
      ...
      plot_c <- ggplot2::ggplot() +
        ggplot2::geom_rect(
          data = data_c, mapping = ggplot2::aes(
            xmin = start, xmax = end,
