...
方法6：使用TIMER解析肿瘤微环境
```{r,message=FALSE, warning=FALSE}
timer<-deconvo_tme(eset = eset_stad,method = "timer",group_list = rep("coad",dim(eset_stad)[2]))
head(timer)
...
```{r}
sig_tme<-calculate_sig_score(pdata = NULL,eset = eset_stad,
...
### 评估代谢相关的signature
```{r}
sig_meta<-calculate_sig_score(pdata = NULL,eset = eset_stad,
...
```{r,message=FALSE}
sig_res<-calculate_sig_score(pdata = NULL,eset = eset_stad,
