...
* **CRAN Compliance**: Fixed policy violations regarding home directory usage and internet access during checks.
  ...
  - Fixed example handling: replaced `if(interactive())` with proper `\dontrun{}` blocks for genuinely non-runable examples (network-dependent, user files required, Suggested packages). CRAN clarified that `if(interactive())` does NOT skip examples in checks.
  - Minimized `\dontrun{}` usage to 22 blocks with documented reasons; functions that can run offline use simulated data.
